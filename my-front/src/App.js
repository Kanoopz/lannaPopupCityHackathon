...
function App() 
{
  ...
  ////////////////////////////////////////////
  let smartContractAddress = "0xF1Bdba08f9D0DE1Eb9CADFcD3663BDC92733f165";
  ...
  async function verifyProofTx()
  {
    // const tx = await smartContractInstance.connect(signer).verifyAndExecute(address, root, nullifierHash, unpackedProof);
    const tx = await smartContractInstance.connect(signer).verifyAndExecute(address, rootParam, nullifierHashParam, proofParam);
  ...
  const verifyProof = async (proof) => 
  {
    ...
    for (i = 0 ; i < unpackedProof.length ; i++)
    {
      console.log(unpackedProof[i]);
      console.log(unpackedProof[i].toString());

      proofArray.push(unpackedProof[i].toString());
    }

    setProofParam(proofArray);
